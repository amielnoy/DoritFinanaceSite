...
export class Base44ContentAdminService implements ContentAdminPort {
  ...
  async createArticle(draft: ArticleDraft): Promise<void> {
    await this.client.entities.BlogPost.create({ ...draft, published: !!draft.published });
  }
  ...
  async createTestimonial(draft: TestimonialDraft): Promise<void> {
    await this.client.entities.Testimonial.create({
      ...draft,
