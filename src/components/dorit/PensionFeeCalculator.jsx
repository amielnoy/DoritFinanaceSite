...
export default function PensionFeeCalculator() {
  ...
  const [depositFee, setDepositFee] = useState(2); // אחוזים
  const [annualFee, setAnnualFee] = useState(0.5); // אחוזים
  const [years, setYears] = useState(25);
  const [annualReturn, setAnnualReturn] = useState(5); // אחוזים
  ...
  const result = useMemo(() => {
    ...
    for (let m = 1; m <= Y * 12; m++) {
      ...
      balance += netDeposit;
      // דמי ניהול שוטפים על היתרה המצטברת
    ...
    // תרחיש אופטימלי: ללא דמי ניהול כלל
    let optimal = 0;
    for (let m = 1; m <= Y * 12; m++) {
...
function ResultCard({ icon: Icon, label, value, highlight }) {
  return (
    <div
